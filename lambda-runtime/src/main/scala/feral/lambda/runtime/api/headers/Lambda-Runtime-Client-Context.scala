...
private[runtime] object `Lambda-Runtime-Client-Context` {
  ...
  implicit val clientContextClientDecoder: Decoder[ClientContextClient] = Decoder.forProduct5(
    ...
    (
      ...
      ClientContextClient(clientId, appTitle, appVersionName, appVersionCode, appPackageName))

  implicit val clientContextEnvDecoder: Decoder[ClientContextEnv] =
    Decoder.forProduct5("platform", "model", "make", "platform_version", "locale")(
      (
          platform: String,
          model: String,
          make: String,
          platformVersion: String,
          locale: String) => ClientContextEnv(platformVersion, platform, make, model, locale))
  ...
  implicit val clientContextDecoder: Decoder[ClientContext] =
    Decoder.forProduct3("client", "custom", "env")(
      (client: ClientContextClient, custom: JsonObject, env: ClientContextEnv) =>
        ClientContext(client, env, custom))
